...
from gym_quadruped.utils.data.h5_dataset import H5Reader
		...
		while frame < len(t_range):
			...
			for g in G.elements:
				...
				for leg_id, leg_name in enumerate(rec_env.legs_order):
					...
					# feet_GRF_geom_ids[f'{g}-{leg_name}'] = render_vector(
					# 	rec_env.viewer,
					...
					# 	geom_id=feet_GRF_geom_ids[f'{g}-{leg_name}'],
					# )
					...
					geom_ids[f'{g}-feet_vec_{leg_name}'] = render_vector(
						...
						color=np.array([0.9, 0.9, 0.9, 0.5]),
...
if __name__ == '__main__':
	path = pathlib.Path('data/aliengo/proprioceptive_data_ep=10_steps=1999.h5')
