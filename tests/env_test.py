...
def test_robot_env(robot_name, terrain_type):
	...
	for obs_name in state_observables_names:
		try:
			...
			raise AssertionError(f'Observable {obs_name} not found in the state for the robot {robot_name}')
		except AssertionError:
			raise AssertionError(
				f'Observable {obs_name} has incorrect shape, expected {env.action_space[obs_name].shape} got {np.asarray(obs_val).shape}'
			)
